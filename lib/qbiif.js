...
export function buildInvoiceIIF(q, meta = {}) {
  ...
  // Owner price override (q.overridePrice): the customer agreed a flat total, so post it as a single
  // line to the Material income item rather than the itemised breakdown (which no longer sums to it).
  const lines = Number(q.overridePrice) > 0
    ? [{ item: qb.items.material, amount: Number(q.overridePrice), desc: "Custom quote (as agreed)" }]
    : [
        ...
        { item: qb.items.engrave, amount: Number(b.engrave) || 0, desc: "Engraving" },
