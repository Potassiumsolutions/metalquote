...
export function buildQuoteSVG(d) {
  ...
  const notes = [d.asCutNote, d.engraveNote, d.notCurrentNote, d.disclaimer, d.leftoverNote, d.shippingNote, d.leadTimeNote, d.validUntil ? `Material pricing on this quote is held through ${d.validUntil}.` : "", d.pricingNote].filter(Boolean);
  const noteLineH = 13;
  ...
  // ---- Previews (optional) — the to-scale PART SIZE (scaling check) + the SHEET LAYOUT, recolored
  // to the quote's light palette. Two-up side by side when both exist, else full width. ----
  const previews = [];
  const pvPart = embedPreview(d.partViewSVG);
  const pvSheet = embedPreview(d.previewSVG);
  if (pvPart) previews.push({ title: "PART — ACTUAL SIZE", ...pvPart });
  if (pvSheet) previews.push({ title: "SHEET LAYOUT", ...pvSheet });
  if (previews.length) {
    y += 44 + 30; // below the total bar
    const availW = W - 2 * M;
    const availH = footerTop - (y + 14) - 16; // room left above the footer line
    if (availH > 70) {
      const cols = previews.length;
      const gap = cols > 1 ? 24 : 0;
      const colW = (availW - gap * (cols - 1)) / cols;
      previews.forEach((p, i) => T.push(txt(M + i * (colW + gap), y, p.title, { size: 9.5, weight: 700, fill: MUTE, spacing: 0.6 })));
      const yBody = y + 14;
      previews.forEach((p, i) => {
        const cx0 = M + i * (colW + gap);
        const s = Math.min(colW / p.vbW, availH / p.vbH);
        const pw = p.vbW * s, ph = p.vbH * s;
        const px = cx0 + (colW - pw) / 2;
        T.push(`<rect x="${px - 10}" y="${yBody - 10}" width="${pw + 20}" height="${ph + 20}" rx="8" fill="${ZEBRA}"/>`);
        T.push(`<svg x="${px}" y="${yBody}" width="${pw}" height="${ph}" viewBox="0 0 ${p.vbW} ${p.vbH}" preserveAspectRatio="xMidYMid meet">${p.inner}</svg>`);
      });
    }
  }
  ...
  if (terms.length) {
    // Start below the notes' ACTUAL height — notes WRAP, so use the wrapped line count
    // (noteLines), not the note COUNT (notes.length), or a wrapped note overlaps the terms.
    let ty = fy + noteLines * noteLineH + 8;
  ...
  // Declare xlink + the Inkscape/Sodipodi namespaces: customer cut files exported from Inkscape or
  // Vectric carry attributes like inkscape:groupmode/label on their <g>s, and the embedded part &
  // sheet previews inline that geometry. Rendering to PDF/JPG parses the SVG as STRICT XML, so an
  // undeclared prefix throws "Namespace prefix … is not defined" and the whole quote fails to render.
...
function embedPreview(svgStr) {
  ...
  const inner = svgStr.slice(open + 1, close)
    ...
    .replace(/var\(--text\)/g, INK)   // the part-size view's dimension labels
    .replace(/var\(--muted\)/g, MUTE); // its dimension lines/ticks
