...
export async function parseSVG(text) {
  ...
  try {
    ...
    const interiorEls = [];
    const interiorRects = []; // client rects of the interior elements → engrave scan region
    ...
    measurable.forEach((el) => {
      ...
      else { interiorUser += len; interiorEls.push(el); interiorRects.push(r); }
    });
    // Union bbox of the interior features, converted client px → user units (via the ratio of the
    // geometry's user-unit bbox to its rendered client rect) → inches. This is the region a raster
    // engrave has to scan line by line when the interior is engraved instead of cut.
    let interiorBoxUser = null;
    if (interiorRects.length) {
      const l = Math.min(...interiorRects.map((r) => r.left)), rt = Math.max(...interiorRects.map((r) => r.right));
      const t = Math.min(...interiorRects.map((r) => r.top)), b = Math.max(...interiorRects.map((r) => r.bottom));
      const cw = rootRect.right - rootRect.left, ch = rootRect.bottom - rootRect.top;
      const kx = cw > 0 ? bbox.width / cw : 0, ky = ch > 0 ? bbox.height / ch : 0;
      interiorBoxUser = { w: (rt - l) * kx, h: (b - t) * ky };
    }
    ...
    return {
      ...
      interiorBBoxIn: interiorBoxUser ? { w: interiorBoxUser.w * scaleX, h: interiorBoxUser.h * scaleY } : null,
