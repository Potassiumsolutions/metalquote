...
export function parseDXF(text) {
  ...
  const flush = () => {
    ...
    if (type === "LINE") {
      ...
      const r = cur[40] || 0;
      // OCS extrusion (0,0,-1): the entity is defined in a mirrored coordinate system, so its world
      // X is negated (see ocsFlipX). A circle is symmetric, so only its center moves.
      const cx = ocsFlipX(cur) ? -cur[10] : cur[10];
      add(cx - r, cur[20] - r);
      add(cx + r, cur[20] + r);
      cutLen += 2 * Math.PI * r;
      pushPoly(arcPts(cx, cur[20], r, 0, 2 * Math.PI, 48), true);
    } else if (type === "ARC") {
      const r = cur[40] || 0;
      ...
      let apts = arcPts(cur[10], cur[20], r, a0, a0 + sweep, Math.max(6, Math.ceil(sweep / (Math.PI / 24))));
      // Arcs use the Object Coordinate System. A (0,0,-1) extrusion (common in mirrored/exported 2D
      // DXFs) flips world X = -OCS X: sample the arc in OCS, then reflect each point. Without this
      // the rounded corners of a mirrored frame land far from their straight edges ("grenaded").
      if (ocsFlipX(cur)) apts = apts.map((p) => [-p[0], p[1]]);
  ...
  const flushPoly = () => {
    ...
    const closed = poly.closed && pts.length > 2;
    // Expand any bulge arcs (LWPOLYLINE/POLYLINE code 42) into sampled points so rounded corners
    // and slot ends measure and draw as true arcs, not straight chords. `drawn` traces the whole
    // outline; when closed it returns to the start vertex, so no separate closing chord is added.
    const drawn = [pts[0]];
    ...
    for (let i = 0; i < segCount; i++) {
      ...
      if (Math.abs(bulge) > 1e-12) {
        const arc = bulgeArcPoints(a, b, bulge);
        for (let k = 1; k < arc.length; k++) drawn.push(arc[k]);
      } else {
        drawn.push(b);
      }
    }
    // LWPOLYLINE/POLYLINE use the OCS: a (0,0,-1) extrusion reflects world X = -OCS X (bulge arcs
    // are already sampled above, so reflecting the final points reverses their sweep correctly).
    const world = poly.ext230 < 0 ? drawn.map((p) => [-p[0], p[1]]) : drawn;
    for (const p of world) add(p[0], p[1]);
    for (let i = 1; i < world.length; i++) {
      cutLen += Math.hypot(world[i][0] - world[i - 1][0], world[i][1] - world[i - 1][1]);
    }
    pushPoly(world, false); // `world` already returns to start when closed
    poly = null;
  };
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    if (code === 0) {
      ...
      if (val === "LWPOLYLINE") {
        flushPoly();
        poly = { pts: [], bulges: [], closed: false, lw: true };
        type = null;
        pendingX = null;
      } else if (val === "POLYLINE") {
        flushPoly();
        poly = { pts: [], bulges: [], closed: false, lw: false };
        type = null;
      } else if (val === "VERTEX") {
    ...
    if (poly && poly.lw) {
      // LWPOLYLINE: repeated 10/20 pairs, 70 = flags (1 => closed), 42 = bulge (arc) for the
      // segment that STARTS at the most recent vertex.
      if (code === 70) poly.closed = (parseInt(val, 10) & 1) === 1;
        ...
        poly.bulges.push(0);
        pendingX = null;
      } else if (code === 42 && poly.bulges.length) {
        poly.bulges[poly.bulges.length - 1] = num;
      } else if (code === 230) poly.ext230 = num; // OCS extrusion Z (negative = mirrored X)
      continue;
    }
    if (type === "VERTEX") {
      if (code === 10) cur[10] = num;
      else if (code === 20 && poly) {
        poly.pts.push([cur[10], num]);
        poly.bulges.push(0);
      } else if (code === 42 && poly && poly.bulges.length) {
        poly.bulges[poly.bulges.length - 1] = num;
      }
      continue;
    }
    if (poly && !poly.lw) {
      if (code === 70) poly.closed = (parseInt(val, 10) & 1) === 1;
      else if (code === 230) poly.ext230 = num; // OCS extrusion Z on the POLYLINE header
      continue;
    }
    if (type === "SPLINE") {
    ...
    // regular entity attributes (230 = OCS extrusion Z, for ARC/CIRCLE mirror detection)
    if ([10, 20, 11, 21, 40, 50, 51, 230].includes(code)) cur[code] = num;
  }
  flush();
  flushPoly();
  ...
  return {
    ...
    // Bounding box of ALL interior features (inches) — the raster-engrave scan region when the
    // customer picks "cut the outer profile + engrave the interior". null = no interior.
    interiorBBoxIn: split.interiorBox
      ? { w: (split.interiorBox.maxX - split.interiorBox.minX) * unitFactor,
          h: (split.interiorBox.maxY - split.interiorBox.minY) * unitFactor }
      : null,
...
// True when an OCS entity (ARC/CIRCLE) carries a negative-Z extrusion (0,0,-1) — the common
// "mirrored 2D" case produced by flips/exports. Under the Arbitrary Axis Algorithm this reflects
// world X = -(OCS X) while Y is unchanged, so callers negate the entity's X. (Arbitrary tilted
// extrusions don't occur in flat laser files, so only the sign of Z matters here.)
function ocsFlipX(cur) {
  return cur[230] != null && cur[230] < 0;
}

// Expand a DXF bulge segment into sampled arc points (raw units), inclusive of both endpoints.
// A bulge b = tan(includedAngle/4): its sign is the sweep direction, magnitude the arc's "fatness".
// Returns [p0 … p1] tracing the true arc so cut length, bbox, and preview all see the curve.
function bulgeArcPoints(p0, p1, bulge) {
  const dx = p1[0] - p0[0], dy = p1[1] - p0[1];
  const chord = Math.hypot(dx, dy);
  const theta = 4 * Math.atan(bulge); // signed included angle
  if (chord < 1e-12 || Math.abs(theta) < 1e-9) return [p0, p1];
  const r = chord / (2 * Math.sin(theta / 2)); // signed radius
  const mx = (p0[0] + p1[0]) / 2, my = (p0[1] + p1[1]) / 2;
  const apothem = r * Math.cos(theta / 2); // signed midpoint→center distance
  const nx = -dy / chord, ny = dx / chord;  // left normal of the chord
  const cx = mx + nx * apothem, cy = my + ny * apothem;
  const rad = Math.abs(r);
  const a0 = Math.atan2(p0[1] - cy, p0[0] - cx);
  const n = Math.max(6, Math.ceil(Math.abs(theta) / (Math.PI / 24))); // ~7.5° per segment
  const pts = [];
  for (let i = 0; i <= n; i++) {
...
function splitOuterInterior(dpaths, bb) {
  ...
  // Union bbox of the interior features (raw units) — the region a raster engrave has to scan.
  let interiorBox = null;
  if (!isFinite(bb.minX)) return { outer, interior, outerLen, interiorLen, interiorBox };
  ...
  for (const d of dpaths) {
    ...
    if (!box || touchesEdge) { outer.push(d); outerLen += len; }
      ...
      interior.push(d); interiorLen += len;
      interiorBox = interiorBox
        ? { minX: Math.min(interiorBox.minX, box.minX), minY: Math.min(interiorBox.minY, box.minY),
            maxX: Math.max(interiorBox.maxX, box.maxX), maxY: Math.max(interiorBox.maxY, box.maxY) }
        : { ...box };
    }
  }
  return { outer, interior, outerLen, interiorLen, interiorBox };
}
