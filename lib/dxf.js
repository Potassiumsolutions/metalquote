...
export function parseDXF(text) {
  ...
  const flushPoly = () => {
    ...
    for (const p of drawn) add(p[0], p[1]);
    for (let i = 1; i < drawn.length; i++) {
      cutLen += Math.hypot(drawn[i][0] - drawn[i - 1][0], drawn[i][1] - drawn[i - 1][1]);
    }
    pushPoly(drawn, false); // `drawn` already returns to start when closed
    poly = null;
  };
