...
export function nestSVG(layout, shape) {
  ...
  // Define the part geometry ONCE in <defs> and stamp it with <use>, instead of inlining the full
  // path per placed part. A busy part (rounded/arc-heavy) is several KB of path data; a full 24×24
  // sheet nests ~200 of them, so inlining produced ~1 MB of SVG that made print-to-PDF crawl.
  // <use> collapses that to one definition + ~200 tiny references. The symbol's viewBox +
  // preserveAspectRatio="none" reproduce the old per-cell nested-<svg> stretch exactly.
  let defs = "";
  let cells;
  if (shape && shape.inner && shape.vb && shape.vb.w > 0 && shape.vb.h > 0) {
    const vb = `${shape.vb.x} ${shape.vb.y} ${shape.vb.w} ${shape.vb.h}`;
    defs = `<defs><symbol id="psu" viewBox="${vb}" preserveAspectRatio="none" class="ps-shape">${shape.inner}</symbol></defs>`;
    cells = layout.rects
      .map((r) => {
        ...
        if (tilt) {
          ...
          return `<g transform="rotate(${tilt.toFixed(2)} ${mx} ${my})">` +
            `<use href="#psu" x="${cx}" y="${cy}" width="${cw}" height="${ch}"/></g>`;
        }
        if (!rotated) {
          return `<use href="#psu" x="${cx}" y="${cy}" width="${cw}" height="${ch}"/>`;
        }
        ...
        return `<g transform="rotate(90 ${mx} ${my})">` +
          `<use href="#psu" x="${ix}" y="${iy}" width="${ch}" height="${cw}"/></g>`;
      })
      .join("");
  } else {
    cells = layout.rects
...
export function nestMultiSVG(sheetWIn, sheetHIn, holderOffsetIn, placements, parts, edgeReserveIn = 0) {
  ...
  // Each DISTINCT part shape is defined once in <defs> (id psu<index>) and stamped with <use> per
  // placement — inlining full geometry per panel bloated the print SVG and slowed print-to-PDF.
  let defs = "";
  (parts || []).forEach((part, idx) => {
    const shape = part && part.shape;
    if (shape && shape.inner && shape.vb && shape.vb.w > 0 && shape.vb.h > 0) {
      const vb = `${shape.vb.x} ${shape.vb.y} ${shape.vb.w} ${shape.vb.h}`;
      defs += `<symbol id="psu${idx}" viewBox="${vb}" preserveAspectRatio="none" class="ps-shape">${shape.inner}</symbol>`;
    }
  });
  ...
  const cells = (placements || [])
    .map((p) => {
      ...
      if (shape && shape.inner && shape.vb && shape.vb.w > 0 && shape.vb.h > 0) {
        ...
        if (!rotated) {
          body = `<use href="#psu${p.i}" x="${cx}" y="${cy}" width="${cw}" height="${ch}"/>`;
        } else {
          ...
          body = `<g transform="rotate(90 ${mx} ${my})">` +
            `<use href="#psu${p.i}" x="${ix}" y="${iy}" width="${ch}" height="${cw}"/></g>`;
        }
      } else {
